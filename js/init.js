    ...
    const handleIntersection = (entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                observer.unobserve(bar); // Unobserve after animation is triggered
            }
        });
    };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Afficher le formulaire au clic sur le bouton "Say Hello"
    sayHelloButton.addEventListener('click', function(event) {
        event.preventDefault();
        helloForm.style.display = 'block';
    });

    // Masquer le formulaire lorsque le formulaire est soumis
    contactForm.addEventListener('submit', function(event) {
        event.preventDefault();
        // Traitez l'envoi du formulaire ici (par exemple, via une requête AJAX)
        alert('Votre message a été envoyé !');
        helloForm.style.display = 'none'; // Masquer le formulaire après l'envoi
    });

    // Masquer le formulaire lorsque l'on clique en dehors de celui-ci
    document.addEventListener('click', function(event) {
        if (!helloForm.contains(event.target) && !sayHelloButton.contains(event.target)) {
